...
class CandidateControllerTest {
    ...
    public void whenRequestCandidatesListPageThenGetPageWithCandidates() {
        var candidate1 = new Candidate(1, "test1", now(),  "desc1", true, 1, 2);
        var candidate2 = new Candidate(2, "test2", now(),  "desc2", false, 3, 4);
    ...
    public void whenPostCandidateWithFileThenSameDataAndRedirectToCandidatesPage() throws Exception {
        var candidate = new Candidate(1, "test1", now(),  "desc1", true, 1, 2);
    ...
    @Test
    public void whenRequestCandidateListAndDeletePageThenGetPageWithCandidates() {
        var candidate1 = new Candidate(1, "test1", now(),  "desc1", true, 1, 2);
        var candidate2 = new Candidate(2, "test2", now(),  "desc2", false, 3, 4);
        var expectedCandidates1 = List.of(candidate1, candidate2);
        var expectedCandidates2 = List.of(candidate2);

        when(candidateService.findAll()).thenReturn(expectedCandidates1);
        when(candidateService.deleteById(1)).thenReturn(true);
        when(candidateService.findAll()).thenReturn(expectedCandidates2);
        ...
        assertThat(view).isEqualTo("redirect:/candidates");
        assertThat(actualCandidates).isEqualTo(expectedCandidates2);
    }
    ...
    public void whenGetById() {
        ...
        var expectedCandidates = List.of(candidate1, candidate2);
        when(candidateService.findAll()).thenReturn(expectedCandidates);
        when(candidateService.findById(2)).thenReturn(Optional.of(candidate2));

        var model = new ConcurrentModel();
        var view = candidateController.getById(model, 2);

        var result = model.getAttribute("candidate");

        assertThat(view).isEqualTo("candidates/one");
        assertThat(result).isEqualTo(candidate2);
