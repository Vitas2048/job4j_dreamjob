...
public class VacancyControllerTest {
    ...
    @Test
    public void whenRequestVacancyListAndDeletePageThenGetPageWithVacancies() {
        var vacancy1 = new Vacancy(1, "test1", now(),  "desc1", true, 1, 2);
        var vacancy2 = new Vacancy(2, "test1", now(),  "desc1", true, 1, 2);
        var expectedVacancies1 = List.of(vacancy1, vacancy2);
        var expectedVacancies2 = List.of(vacancy2);

        when(vacancyService.findAll()).thenReturn(expectedVacancies1);
        when(vacancyService.deleteById(1)).thenReturn(true);
        when(vacancyService.findAll()).thenReturn(expectedVacancies2);
        ...
        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(actualVacancies).isEqualTo(expectedVacancies2);
    }
    ...
    public void whenGetById() {
        ...
        var expectedVacancies1 = List.of(vacancy1, vacancy2);
        when(vacancyService.findAll()).thenReturn(expectedVacancies1);
        when(vacancyService.findById(2)).thenReturn(Optional.of(vacancy2));

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, 2);

        var result = model.getAttribute("vacancy");
